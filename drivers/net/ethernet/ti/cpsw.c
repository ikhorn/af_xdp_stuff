...
static void cpsw_destroy_page_pool(struct cpsw_common *cpsw)
{
	struct page_pool *pool = cpsw->rx_page_pool;
	struct page *page;
	int i;

	for (i = 0; i < descs_pool_size; i++) {
		page = page_pool_dev_alloc_pages(pool);
		if (!page)
			break;
		...
		dma_unmap_single(cpsw->dev, page->dma_addr, cpsw->rx_packet_max,
				 DMA_FROM_DEVICE);
